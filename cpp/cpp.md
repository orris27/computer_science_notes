...
6. 文件描述符
    ...
    2. dup2:(dup to)复制文件描述符为指定的文件描述符
        ...
        3. 可以重定向stdout
            1. 用途:实现bash中的管道
                1. 以`ls -l | wc -l`为例,本质的思想是2个进程+管道+重定向
                    1. 第一个进程用dup2将stdout指向管道的写端,用exec函数族调用可执行程序`ls`
                    2. 第二个进程用dup2将stdin指向管道的读端,用exec函数调用可执行程序`wc`
...
11. 信号
    ...
    8. 发送信号传参
        1. sigqueue:
            1. 注意
                1. sival_ptr在不同进程间是没有意义的,只有进程中才有意义的
                    1. 进程中只有虚拟地址,不用真正的物理内存的地址
                    2. 如果是IPC情况,传递给对方的是我们自己进程的虚拟地址
                    3. 所以是没用的.只能在自己的进程里用
...
18. offsetof
    1. 获取某个成员在类型中的偏移位置:`offsetof(struct sockaddr_un,sun_path)`
        + ((int)(&((type*)0)->member))
#### 4-1-1. 文件
1. 特点
