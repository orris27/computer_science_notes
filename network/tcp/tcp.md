...
2. 说明
    ...
    2. 16位目的端口号
    3. 32位序列号
        1. 表示数据的第一个字节在整个文件中的序号
            1. 例子:假设我们要传输100KB的文件α,四等分.第一次的TCP包的序列号为0,表示数据内第一个字节的数据是α的第0个字节.第二个TCP包的序列号是`25*1024=25600`,表示是数据内第一个字节的数据是α的第25600个字节.
    4. 32位确认号
        1. 表示期望对方下一次发送的数据的第一个字节的数据的序号是多少
        2. 作用:确认+疯狂暗示
    5. 4位首部长度
        1. 表示TCP首部的大小是4字节的多少倍<=>TCP包中数据的起始位置
        2. 4位=>15为max=>15\*4=60字节为TCP首部的最大大小(一般TCP首部是20字节)
    6. 6位保留
    7. 6位标志位
        1. URG：紧急指针有效
        2. ACK：确认序号有效
        ...
        6. FIN：表示将要终止一个连接
    8. 16位窗口大小
        1. 发送方告诉接收方自己能接收的最大数据大小<=>[当前包的确认号,当前包的确认号+窗口大小]
        2. 作用:流量控制
            1. 原理:接收方丢弃超过窗口大小的数据.由于接收方包括主机和路由器,路由器的缓存区大小也有限制=>更好发挥网络功能,发送方发送的数据其实也有大小限制
        3. 概念:
            1. 通告接收窗口(rwnd):接收方窗口的大小
            2. 拥塞窗口(cwnd):发送方在一个RTT内可以最多发送的数据包数,主要是为了避免超过网络的承受能力
            3. 慢启动阶段:cwnd从1开始按指数增长知道ssthresh,之后+1直到拥塞
                1. 目的:发送方一开始不知道网络承受能力多大,所以要从1开始不断测试网络的承受能力
